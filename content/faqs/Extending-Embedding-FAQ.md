...
请注意由于 [`PyObject_CallObject()`](https://docs.python.org/zh-cn/3.11/c-api/call.html#c.PyObject_CallObject) *总是* 接受一个元组作为参数列表，要调用不带参数的函数，则传入格式为 "()"，要调用只带一个参数的函数，则应将参数包含于圆括号中，例如 "(i)"。
