...
`__dir__` 函数应当不接受任何参数，并且返回一个表示模块中可访问名称的字符串序列。 此函数如果存在，将会重载一个模块中的标准 [`dir()`](https://docs.python.org/zh-cn/3.11/library/functions.html#dir) 查找。
...
*__slots__* 允许我们显式地声明数据成员（如特征属性）并禁止创建 [`__dict__`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#object.__dict__) 和 *__weakref__* (除非是在 *__slots__* 中显式地声明或是在父类中可用。)
...
类主体会以（类似于） `exec(body, globals(), namespace)` 的形式被执行。普通调用与 [`exec()`](https://docs.python.org/zh-cn/3.11/library/functions.html#exec) 的关键区别在于当类定义发生于函数内部时，词法作用域允许类主体（包括任何方法）引用来自当前和外部作用域的名称。
...
如果类主体中有任何方法引用了 `__class__` 或 `super`，这个类对象会通过零参数形式的 [`super()`](https://docs.python.org/zh-cn/3.11/library/functions.html#super). `__class__` 所引用，这是由编译器所创建的隐式闭包引用。这使用零参数形式的 [`super()`](https://docs.python.org/zh-cn/3.11/library/functions.html#super) 能够正确标识正在基于词法作用域来定义的类，而被用于进行当前调用的类或实例则是基于传递给方法的第一个参数来标识的。

**CPython 实现细节：** 在 CPython 3.6 及之后的版本中，`__class__` 单元会作为类命名空间中的 `__classcell__` 条目被传给元类。 如果存在，它必须被向上传播给 `type.__new__` 调用，以便能正确地初始化该类。 如果不这样做，在 Python 3.8 中将引发 [`RuntimeError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#RuntimeError)。

当使用默认的元类 [`type`](https://docs.python.org/zh-cn/3.11/library/functions.html#type)，或者任何最终会调用 `type.__new__` 的元类时，以下额外的自定义步骤将在创建类对象之后被发起调用:

1. `type.__new__` 方法会收集类命名空间中所有定义了 [`__set_name__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__set_name__) 方法的属性;
2. 这些 `__set_name__` 方法将附带所定义的类和指定的属性所赋的名称进行调用;
3. 在新类基于方法解析顺序所确定的直接父类上调用 [`__init_subclass__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__init_subclass__) 钩子。

在类对象创建之后，它会被传给包含在类定义中的类装饰器（如果有的话），得到的对象将作为已定义的类绑定到局部命名空间。

当通过 `type.__new__` 创建一个新类时，提供以作为命名空间形参的对象会被复制到一个新的有序映射并丢弃原对象。这个新副本包装于一个只读代理中，后者则成为类对象的 [`__dict__`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#object.__dict__) 属性。
...
以下方法被用来重载 [`isinstance()`](https://docs.python.org/zh-cn/3.11/library/functions.html#isinstance) 和 [`issubclass()`](https://docs.python.org/zh-cn/3.11/library/functions.html#issubclass) 内置函数的默认行为。

特别地，元类 [`abc.ABCMeta`](https://docs.python.org/zh-cn/3.11/library/abc.html#abc.ABCMeta) 实现了这些方法以便允许将抽象基类（ABC）作为“虚拟基类”添加到任何类或类型（包括内置类型），包括其他 ABC 之中。
...
- [**PEP 3119**](https://peps.python.org/pep-3119/) - 引入抽象基类
  ...
  新增功能描述，通过 [`__instancecheck__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#class.__instancecheck__) 和 [`__subclasscheck__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#class.__subclasscheck__) 来定制 [`isinstance()`](https://docs.python.org/zh-cn/3.11/library/functions.html#isinstance) 和 [`issubclass()`](https://docs.python.org/zh-cn/3.11/library/functions.html#issubclass) 行为，加入此功能的动机是出于向该语言添加抽象基类的内容（参见 [`abc`](https://docs.python.org/zh-cn/3.11/library/abc.html#module-abc) 模块）。

### 3.3.5. 模拟泛型类型

当使用 [类型标注](https://docs.python.org/zh-cn/3.11/glossary.html#term-annotation) 时，使用 Python 的方括号标记来 *形参化* 一个 [generic type](https://docs.python.org/zh-cn/3.11/glossary.html#term-generic-type) 往往会很有用处。 例如，`list[int]` 这样的标注可以被用来表示一个 [`list`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#list) 中的所有元素均为 [`int`](https://docs.python.org/zh-cn/3.11/library/functions.html#int) 类型。
...
- [泛用别名类型](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#types-genericalias)
...
- *classmethod* object.**__class_getitem__**(*cls*, *key*)
  ...
  按照 *key* 参数指定的类型返回一个表示泛型类的专门化对象。当在类上定义时，`__class_getitem__()` 会自动成为类方法。 因此，当它被定义时没有必要使用 [`@classmethod`](https://docs.python.org/zh-cn/3.11/library/functions.html#classmethod) 来装饰。

#### 3.3.5.1. *__class_getitem__* 的目的

[`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__) 的目的是允许标准库泛型类的运行时形参化以更方便地对这些类应用 [类型提示](https://docs.python.org/zh-cn/3.11/glossary.html#term-type-hint)。

要实现可以在运行时被形参化并可被静态类型检查所理解的自定义泛型类，用户应当从已经实现了 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__) 的标准库类继承，或是从 [`typing.Generic`](https://docs.python.org/zh-cn/3.11/library/typing.html#typing.Generic) 继承，这个类拥有自己的 `__class_getitem__()` 实现。

标准库以外的类上的 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__) 自定义实现可能无法被第三方类型检查器如 mypy 所理解。 不建议在任何类上出于类型提示以外的目的使用 `__class_getitem__()`。



#### 3.3.5.2. *__class_getitem__* 与 *__getitem__*

通常，使用方括号语法 [抽取](https://docs.python.org/zh-cn/3.11/reference/expressions.html#subscriptions) 一个对象将会调用在该对象的类上定义的 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 实例方法。 不过，如果被拟抽取的对象本身是一个类，则可能会调用 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__) 类方法。 `__class_getitem__()` 如果被正确地定义，则应当返回一个 [GenericAlias](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#types-genericalias) 对象。

使用 [expression](https://docs.python.org/zh-cn/3.11/glossary.html#term-expression) `obj[x]` 来呈现，Python 解释器会遵循下面这样的过程来确定应当调用 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 还是 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__):
...
在 Python 中，所有的类自身也是其他类的实例。 一个类所属的类被称为该类的 [metaclass](https://docs.python.org/zh-cn/3.11/glossary.html#term-metaclass)，并且大多数类都将 [`type`](https://docs.python.org/zh-cn/3.11/library/functions.html#type) 类作为它们的元类。 [`type`](https://docs.python.org/zh-cn/3.11/library/functions.html#type) 没有定义 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__)，这意味着 `list[int]`, `dict[str, float]` 和 `tuple[str, bytes]` 这样的表达式都将导致 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__) 被调用:
...
然而，如果一个类属于定义了 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 的自定义元类，则抽取该类可能导致不同的行为。 这方面的一个例子可以在 [`enum`](https://docs.python.org/zh-cn/3.11/library/enum.html#module-enum) 模块中找到:
...
- [**PEP 560**](https://peps.python.org/pep-0560/) - 对 typing 模块和泛型的核心支持
  ...
  介绍 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__)，并指明 [抽取](https://docs.python.org/zh-cn/3.11/reference/expressions.html#subscriptions) 在何时会导致 `__class_getitem__()` 而不是 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 被调用
...
可以定义下列方法来实现容器对象。 窗口通常属于 [序列](https://docs.python.org/zh-cn/3.11/glossary.html#term-sequence) (如 [`列表`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#list) 或 [`元组`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#tuple)) 或者 [映射](https://docs.python.org/zh-cn/3.11/glossary.html#term-mapping) (如 [`字典`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#dict))，但也有表现为其他形式的容器。 前几个方法被用来模拟序列或是模拟映射；两者的不同之处在于序列允许的键应为整数 *k* 并且 `0 <= k < N` 其中 *N* 是序列或 [`slice`](https://docs.python.org/zh-cn/3.11/library/functions.html#slice) 对象的长度，它们定义了条目的范围。 此外还建议让映射提供 `keys()`, `values()`, `items()`, `get()`, `clear()`, `setdefault()`, `pop()`, `popitem()`, `copy()` 以及 `update()` 等方法，它们的行为应与 Python 的标准 [`字典`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#dict) 对象类似。 此外 [`collections.abc`](https://docs.python.org/zh-cn/3.11/library/collections.abc.html#module-collections.abc) 模块提供了一个 [`MutableMapping`](https://docs.python.org/zh-cn/3.11/library/collections.abc.html#collections.abc.MutableMapping) [abstract base class](https://docs.python.org/zh-cn/3.11/glossary.html#term-abstract-base-class) 以便根据由 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__), [`__setitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__setitem__), [`__delitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__delitem__) 和 `keys()` 组成的基本集来创建所需的方法。 可变序列还应提供 `append()`, `count()`, `index()`, `extend()`, `insert()`, `pop()`, `remove()`, `reverse()` 和 `sort()` 等方法，就像 Python 的标准 [`list`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#list) 对象那样。 最后，序列类型还应通过定义下文描述的 [`__add__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__add__), [`__radd__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__radd__), [`__iadd__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__iadd__), [`__mul__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__mul__), [`__rmul__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__rmul__) 和 [`__imul__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__imul__) 等方法来实现加法（指拼接）和乘法（指重复）；它们不应定义其他数值运算符。 此外还建议映射和序列都实现 [`__contains__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__contains__) 方法以允许高效地使用 `in` 运算符；对于映射，`in` 应当搜索映射的键；对于序列，则应当搜索其中的值。 另外还建议映射和序列都实现 [`__iter__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__iter__) 方法以允许高效地迭代容器中的条目；对于映射，`__iter__()` 应当迭代对象的键；对于序列，则应当迭代其中的值。

- object.**__len__**(*self*)

  调用此方法以实现内置函数 [`len()`](https://docs.python.org/zh-cn/3.11/library/functions.html#len)。应该返回对象的长度，以一个 `>=` 0 的整数表示。此外，如果一个对象未定义 [`__bool__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__bool__) 方法而其 [`__len__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__len__) 方法返回值为零，则在布尔运算中会被视为假值。**CPython 实现细节：** 在 CPython 中，要求长度最大为 [`sys.maxsize`](https://docs.python.org/zh-cn/3.11/library/sys.html#sys.maxsize)。如果长度大于 `sys.maxsize` 则某些特性 (例如 [`len()`](https://docs.python.org/zh-cn/3.11/library/functions.html#len)) 可能会引发 [`OverflowError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#OverflowError)。要通过真值检测来防止引发 `OverflowError`，对象必须定义 [`__bool__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__bool__) 方法。

- object.**__length_hint__**(*self*)

  调用此方法以实现 [`operator.length_hint()`](https://docs.python.org/zh-cn/3.11/library/operator.html#operator.length_hint)。 应该返回对象长度的估计值（可能大于或小于实际长度）。 此长度应为一个 `>=` 0 的整数。 返回值也可以为 [`NotImplemented`](https://docs.python.org/zh-cn/3.11/library/constants.html#NotImplemented)，这会被视作与 `__length_hint__` 方法完全不存在时一样处理。 此方法纯粹是为了优化性能，并不要求正确无误。*3.4 新版功能.*
...
- object.**__getitem__**(*self*, *key*)
  ...
  调用此方法以实现 `self[key]` 的未付。 对于 [sequence](https://docs.python.org/zh-cn/3.11/glossary.html#term-sequence) 类型，接受的键应为整数和切片对象。 请注意负数索引（如果类想要模拟 [sequence](https://docs.python.org/zh-cn/3.11/glossary.html#term-sequence) 类型）的特殊解读是依赖于 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 方法。 如果 *key* 的类型不正确，则会引发 [`TypeError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#TypeError)；如果为序列索引集范围以外的值（在进行任何负数索引的特殊解读之后），则应当引发 [`IndexError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#IndexError)。 对于 [mapping](https://docs.python.org/zh-cn/3.11/glossary.html#term-mapping) 类型，如果 *key* 找不到（不在容器中），则应当引发 [`KeyError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#KeyError)。备注 [`for`](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#for) 循环在有不合法索引时会期待捕获 [`IndexError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#IndexError) 以便正确地检测到序列的结束。备注 当 [抽取](https://docs.python.org/zh-cn/3.11/reference/expressions.html#subscriptions) 一个 *class* 时，可能会调用特殊类方法 [`__class_getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__class_getitem__) 而不是 `__getitem__()`。 请参阅 [__class_getitem__ 与 __getitem__](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#classgetitem-versus-getitem) 了解详情。

- object.**__setitem__**(*self*, *key*, *value*)

  调用此方法以实现向 `self[key]` 赋值。注意事项与 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 相同。为对象实现此方法应该仅限于需要映射允许基于键修改值或添加键，或是序列允许元素被替换时。不正确的 *key* 值所引发的异常应与 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 方法的情况相同。

- object.**__delitem__**(*self*, *key*)

  调用此方法以实现 `self[key]` 的删除。注意事项与 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 相同。为对象实现此方法应该权限于需要映射允许移除键，或是序列允许移除元素时。不正确的 *key* 值所引发的异常应与 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 方法的情况相同。

- object.**__missing__**(*self*, *key*)

  此方法由 [`dict`](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#dict).[`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 在找不到字典中的键时调用以实现 dict 子类的 `self[key]`。

- object.**__iter__**(*self*)

  此方法会在需要为一个容器创建 [iterator](https://docs.python.org/zh-cn/3.11/glossary.html#term-iterator) 时被调用。 此方法应当返回一个新的迭代器对象，它可以对容器中的所有对象执行迭代。 对于映射，它应当对窗口中的键执行迭代。

- object.**__reversed__**(*self*)

  此方法（如果存在）会被 [`reversed()`](https://docs.python.org/zh-cn/3.11/library/functions.html#reversed) 内置函数调用以实现逆向迭代。它应当返回一个新的以逆序逐个迭代容器内所有对象的迭代器对象。如果未提供 [`__reversed__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__reversed__) 方法，则 [`reversed()`](https://docs.python.org/zh-cn/3.11/library/functions.html#reversed) 内置函数将回退到使用序列协议 ([`__len__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__len__) 和 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__))。支持序列协议的对象应当仅在能够提供比 [`reversed()`](https://docs.python.org/zh-cn/3.11/library/functions.html#reversed) 所提供的实现更高效的实现时才提供 [`__reversed__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__reversed__) 方法。

成员检测运算符 ([`in`](https://docs.python.org/zh-cn/3.11/reference/expressions.html#in) 和 [`not in`](https://docs.python.org/zh-cn/3.11/reference/expressions.html#not-in)) 通常以对容器进行逐个迭代的方式来实现。 不过，容器对象可以提供以下特殊方法并采用更有效率的实现，这样也不要求对象必须为可迭代对象。

- object.**__contains__**(*self*, *item*)

  调用此方法以实现成员检测运算符。如果 *item* 是 *self* 的成员则应返回真，否则返回假。对于映射类型，此检测应基于映射的键而不是值或者键值对。对于未定义 [`__contains__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__contains__) 的对象，成员检测将首先尝试通过 [`__iter__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__iter__) 进行迭代，然后再使用 [`__getitem__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getitem__) 的旧式序列迭代协议，参看 [语言参考中的相应部分](https://docs.python.org/zh-cn/3.11/reference/expressions.html#membership-test-details)。
...
- object.**__or__**(*self*, *other*)
  ...
  These methods are called to implement the binary arithmetic operations (`+`, `-`, `*`, `@`, `/`, `//`, `%`, [`divmod()`](https://docs.python.org/zh-cn/3.11/library/functions.html#divmod), [`pow()`](https://docs.python.org/zh-cn/3.11/library/functions.html#pow), `**`, `<<`, `>>`, `&`, `^`, `|`). For instance, to evaluate the expression `x + y`, where *x* is an instance of a class that has an [`__add__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__add__) method, `type(x).__add__(x, y)` is called. The [`__divmod__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__divmod__) method should be the equivalent to using [`__floordiv__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__floordiv__) and [`__mod__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__mod__); it should not be related to [`__truediv__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__truediv__). Note that [`__pow__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__pow__) should be defined to accept an optional third argument if the ternary version of the built-in [`pow()`](https://docs.python.org/zh-cn/3.11/library/functions.html#pow) function is to be supported.如果这些方法中的某一个不支持与所提供参数进行运算，它应该返回 `NotImplemented`。
...
- object.**__ror__**(*self*, *other*)
  ...
  These methods are called to implement the binary arithmetic operations (`+`, `-`, `*`, `@`, `/`, `//`, `%`, [`divmod()`](https://docs.python.org/zh-cn/3.11/library/functions.html#divmod), [`pow()`](https://docs.python.org/zh-cn/3.11/library/functions.html#pow), `**`, `<<`, `>>`, `&`, `^`, `|`) with reflected (swapped) operands. These functions are only called if the left operand does not support the corresponding operation [3](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#id10) and the operands are of different types. [4](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#id11) For instance, to evaluate the expression `x - y`, where *y* is an instance of a class that has an [`__rsub__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__rsub__) method, `type(y).__rsub__(y, x)` is called if `type(x).__sub__(x, y)` returns *NotImplemented*.请注意三元版的 [`pow()`](https://docs.python.org/zh-cn/3.11/library/functions.html#pow) 并不会尝试调用 [`__rpow__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__rpow__) (因为强制转换规则会太过复杂)。备注 如果右操作数类型为左操作数类型的一个子类，且该子类提供了指定运算的反射方法，则此方法将先于左操作数的非反射方法被调用。 此行为可允许子类重载其祖先类的运算符。
...
- object.**__ior__**(*self*, *other*)
  ...
  调用这些方法来实现扩展算术赋值 (`+=`, `-=`, `*=`, `@=`, `/=`, `//=`, `%=`, `**=`, `<<=`, `>>=`, `&=`, `^=`, `|=`)。这些方法应该尝试进行自身操作 (修改 *self*) 并返回结果 (结果应该但并非必须为 *self*)。如果某个方法未被定义，相应的扩展算术赋值将回退到普通方法。例如，如果 *x* 是具有 [`__iadd__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__iadd__) 方法的类的一个实例，则 `x += y` 就等价于 `x = x.__iadd__(y)`。否则就如 `x + y` 的求值一样选择 `x.__add__(y)` 和 `y.__radd__(x)`。在某些情况下，扩展赋值可导致未预期的错误 (参见 [为什么 a_tuple[i\] += ['item'] 会引发异常？](https://docs.python.org/zh-cn/3.11/faq/programming.html#faq-augmented-assignment-tuple-error))，但此行为实际上是数据模型的一个组成部分。
...
- object.**__invert__**(*self*)
  ...
  调用此方法以实现一元算术运算 (`-`, `+`, [`abs()`](https://docs.python.org/zh-cn/3.11/library/functions.html#abs) 和 `~`)。
...
- object.**__float__**(*self*)
  ...
  调用这些方法以实现内置函数 [`complex()`](https://docs.python.org/zh-cn/3.11/library/functions.html#complex), [`int()`](https://docs.python.org/zh-cn/3.11/library/functions.html#int) 和 [`float()`](https://docs.python.org/zh-cn/3.11/library/functions.html#float)。应当返回一个相应类型的值。

- object.**__index__**(*self*)

  调用此方法以实现 [`operator.index()`](https://docs.python.org/zh-cn/3.11/library/operator.html#operator.index) 以及 Python 需要无损地将数字对象转换为整数对象的场合（例如切片或是内置的 [`bin()`](https://docs.python.org/zh-cn/3.11/library/functions.html#bin), [`hex()`](https://docs.python.org/zh-cn/3.11/library/functions.html#hex) 和 [`oct()`](https://docs.python.org/zh-cn/3.11/library/functions.html#oct) 函数)。 存在此方法表明数字对象属于整数类型。 必须返回一个整数。如果未定义 [`__int__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__int__), [`__float__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__float__) 和 [`__complex__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__complex__) 则相应的内置函数 [`int()`](https://docs.python.org/zh-cn/3.11/library/functions.html#int), [`float()`](https://docs.python.org/zh-cn/3.11/library/functions.html#float) 和 [`complex()`](https://docs.python.org/zh-cn/3.11/library/functions.html#complex) 将回退为 [`__index__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__index__)。
...
- object.**__ceil__**(*self*)
  ...
  调用这些方法以实现内置函数 [`round()`](https://docs.python.org/zh-cn/3.11/library/functions.html#round) 以及 [`math`](https://docs.python.org/zh-cn/3.11/library/math.html#module-math) 函数 [`trunc()`](https://docs.python.org/zh-cn/3.11/library/math.html#math.trunc), [`floor()`](https://docs.python.org/zh-cn/3.11/library/math.html#math.floor) 和 [`ceil()`](https://docs.python.org/zh-cn/3.11/library/math.html#math.ceil)。 除了将 *ndigits* 传给 `__round__()` 的情况之外这些方法的返回值都应当是原对象截断为 [`Integral`](https://docs.python.org/zh-cn/3.11/library/numbers.html#numbers.Integral) (通常为 [`int`](https://docs.python.org/zh-cn/3.11/library/functions.html#int))。如果 [`__int__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__int__) 或 [`__index__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__index__) 均未被定义则内置函数 [`int()`](https://docs.python.org/zh-cn/3.11/library/functions.html#int) 会回退至 [`__trunc__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__trunc__)。*在 3.11 版更改:* The delegation of [`int()`](https://docs.python.org/zh-cn/3.11/library/functions.html#int) to [`__trunc__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__trunc__) is deprecated.



### 3.3.9. with 语句上下文管理器

*上下文管理器* 是一个对象，它定义了在执行 [`with`](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#with) 语句时要建立的运行时上下文。 上下文管理器处理进入和退出所需运行时上下文以执行代码块。 通常使用 `with` 语句（在 [with 语句](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#with) 中描述），但是也可以通过直接调用它们的方法来使用。

上下文管理器的典型用法包括保存和恢复各种全局状态，锁定和解锁资源，关闭打开的文件等等。

要了解上下文管理器的更多信息，请参阅 [上下文管理器类型](https://docs.python.org/zh-cn/3.11/library/stdtypes.html#typecontextmanager)。

- object.**__enter__**(*self*)

  进入与此对象相关的运行时上下文。 [`with`](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#with) 语句将会绑定这个方法的返回值到 `as` 子句中指定的目标，如果有的话。

- object.**__exit__**(*self*, *exc_type*, *exc_value*, *traceback*)

  退出关联到此对象的运行时上下文。 各个参数描述了导致上下文退出的异常。 如果上下文是无异常地退出的，三个参数都将为 [`None`](https://docs.python.org/zh-cn/3.11/library/constants.html#None)。如果提供了异常，并且希望方法屏蔽此异常（即避免其被传播），则应当返回真值。 否则的话，异常将在退出此方法时按正常流程处理。请注意 [`__exit__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__exit__) 方法不应该重新引发被传入的异常，这是调用者的责任。
...
举例来说，如果 `MyClass.__match_args__` 为 `("left", "center", "right")` 则意味着 `case MyClass(x, y)` 就等价于 `case MyClass(left=x, center=y)`。 请注意模式中参数的数量必须小于等于 *__match_args__* 中元素的数量；如果前者大于后者，则尝试模式匹配时将引发 [`TypeError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#TypeError)。
...
此行为背后的原理在于包括类型对象在内的所有对象都会实现的几个特殊方法如 [`__hash__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__hash__) 和 [`__repr__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__repr__)。 如果这些方法的隐式查找使用了传统的查找过程，则当它们在对类型对象自身发起调用时将会失败:
...
除了出于正确性考虑而会绕过任何实例属性，隐式特殊方法查找通常还会绕过 [`__getattribute__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getattribute__) 方法，甚至包括对象的元类:
...
以这种方式绕过 [`__getattribute__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__getattribute__) 机制为解释器内部的速度优化提供了显著的空间，其代价则是牺牲了一些处理特殊方法时的灵活性（特殊方法 *must* 必须设置在类对象自身上以便始终一致地由解释器发起调用）。
...
[awaitable](https://docs.python.org/zh-cn/3.11/glossary.html#term-awaitable) 对象主要实现了 [`__await__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__await__) 方法。 从 [`async def`](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#async-def) 函数返回的 [协程对象](https://docs.python.org/zh-cn/3.11/glossary.html#term-coroutine) 即为可等待对象。

备注

 

The [generator iterator](https://docs.python.org/zh-cn/3.11/glossary.html#term-generator-iterator) objects returned from generators decorated with [`types.coroutine()`](https://docs.python.org/zh-cn/3.11/library/types.html#types.coroutine) are also awaitable, but they do not implement [`__await__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__await__).

- object.**__await__**(*self*)

  必须返回一个 [iterator](https://docs.python.org/zh-cn/3.11/glossary.html#term-iterator)。 应当被用来实现 [awaitable](https://docs.python.org/zh-cn/3.11/glossary.html#term-awaitable) 对象。 例如，[`asyncio.Future`](https://docs.python.org/zh-cn/3.11/library/asyncio-future.html#asyncio.Future) 实现了此方法以与 [`await`](https://docs.python.org/zh-cn/3.11/reference/expressions.html#await) 表达式相兼容。备注 The language doesn't place any restriction on the type or value of the objects yielded by the iterator returned by `__await__`, as this is specific to the implementation of the asynchronous execution framework (e.g. [`asyncio`](https://docs.python.org/zh-cn/3.11/library/asyncio.html#module-asyncio)) that will be managing the [awaitable](https://docs.python.org/zh-cn/3.11/glossary.html#term-awaitable) object.
...
[协程对象](https://docs.python.org/zh-cn/3.11/glossary.html#term-coroutine) 属于 [awaitable](https://docs.python.org/zh-cn/3.11/glossary.html#term-awaitable) 对象。 协程的执行可以通过调用 [`__await__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__await__) 并迭代其结果来控制。 当协程结束执行并返回时，迭代器会引发 [`StopIteration`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#StopIteration)，而该异常的 `value` 属性将存放返回值。 如果协程引发了异常，它会被迭代器传播出去。 协程不应当直接引发未被处理的 [`StopIteration`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#StopIteration) 异常。

协程也具有下面列出的方法，它们类似于生成器的对应方法 (参见 [生成器-迭代器的方法](https://docs.python.org/zh-cn/3.11/reference/expressions.html#generator-methods))。 但是，与生成器不同，协程并不直接支持迭代。

*在 3.5.2 版更改:* 等待一个协程超过一次将引发 [`RuntimeError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#RuntimeError)。

- coroutine.**send**(*value*)

  开始或恢复协程的执行。 如果 *value* 为 `None`，那么这就相当于前往 [`__await__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__await__) 所返回迭代器的下一项。 如果 *value* 不为 `None`，此方法将委托给导致协程挂起的迭代器的 [`send()`](https://docs.python.org/zh-cn/3.11/reference/expressions.html#generator.send) 方法。 其结果（返回值，[`StopIteration`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#StopIteration) 或是其他异常）将与上述对 `__await__()` 返回值进行迭代的结果相同。

- coroutine.**throw**(*value*)

- coroutine.**throw**(*type*[, *value*[, *traceback*]])

  在协程内引发指定的异常。 此方法将委托给导致该协程挂起的迭代器的 [`throw()`](https://docs.python.org/zh-cn/3.11/reference/expressions.html#generator.throw) 方法，如果存在此方法的话。 否则，该异常将在挂起点被引发。 其结果（返回值，[`StopIteration`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#StopIteration) 或是其他异常）将与上述对 [`__await__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__await__) 返回值进行迭代的结果相同。 如果该异常未在协程内被捕获，则将回传给调用方。

- coroutine.**close**()

  此方法会使得协程清理自身并退出。 如果协程被挂起，此方法会先委托给导致协程挂起的迭代器的 [`close()`](https://docs.python.org/zh-cn/3.11/reference/expressions.html#generator.close) 方法，如果存在该方法。 然后它会在挂起点引发 [`GeneratorExit`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#GeneratorExit)，使得协程立即清理自身。 最后，协程会被标记为已结束执行，即使它根本未被启动。当协程对象将要被销毁时，会使用以上处理过程来自动关闭。
...
异步迭代器可在 [`async for`](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#async-for) 语句中使用。
...
- object.**__anext__**(*self*)
  ...
  必须返回一个 *可迭代对象* 输出迭代器的下一结果值。 当迭代结束时应该引发 [`StopAsyncIteration`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#StopAsyncIteration) 错误。
...
*在 3.7 版更改:* 在 Python 3.7 之前，[`__aiter__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__aiter__) 可以返回一个 *可等待对象* 并将被解析为 [异步迭代器](https://docs.python.org/zh-cn/3.11/glossary.html#term-asynchronous-iterator)。

从 Python 3.7 开始，[`__aiter__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__aiter__) 必须返回一个异步迭代器对象。 返回任何其他对象都将导致 [`TypeError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#TypeError) 错误。
...
异步上下文管理器可在 [`async with`](https://docs.python.org/zh-cn/3.11/reference/compound_stmts.html#async-with) 语句中使用。

- object.**__aenter__**(*self*)

  在语义上类似于 [`__enter__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__enter__)，仅有的区别是它必须返回一个 *可等待对象*。

- object.**__aexit__**(*self*, *exc_type*, *exc_value*, *traceback*)

  在语义上类似于 [`__exit__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__exit__)，仅有的区别是它必须返回一个 *可等待对象*。
...
- [1](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#id1)

  在某些情况下 *有可能* 基于可控的条件改变一个对象的类型。 但这通常不是个好主意，因为如果处理不当会导致一些非常怪异的行为。

- [2](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#id2)

  [`__hash__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__hash__), [`__iter__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__iter__), [`__reversed__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__reversed__) 以及 [`__contains__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__contains__) 方法对此有特殊处理；其他方法仍会引发 [`TypeError`](https://docs.python.org/zh-cn/3.11/library/exceptions.html#TypeError)，但可能依赖 `None` 属于不可调用对象的行为来做到这一点。

- [3](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#id5)

  这里的“不支持”是指该类无此方法，或方法返回 `NotImplemented`。 如果你想强制回退到右操作数的反射方法，请不要设置方法为 `None` — 那会造成显式地 *阻塞* 此种回退的相反效果。

- [4](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#id6)

  对于相同类型的操作数，如果非返回方法 -- 例如 [`__add__()`](https://docs.python.org/zh-cn/3.11/reference/datamodel.html#object.__add__) -- 失败则会认为整个运算都不被支持，这就是反射方法不会被调用的原因。